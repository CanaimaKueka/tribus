...
class CloudViews(TestCase):
    ...
    def test_cloud_profile(self):
        ...
        details_i386 = {'Version': '2.63a-1', 'Architecture':'i386',
            'Size': '21957', 'InstalledSize': '55328',
            'MD5sum': '3917d6227e61324a29459303c0531587',
            'Filename':'pool/main/b/blender/blender_2.63a-1_i386.deb',
            'Distribution': "kukenan"
        }
        
        details_amd64 = {'Version': '2.63a-1', 'Architecture':'amd64',
            'Size': '21886', 'InstalledSize': '57306',
        ...
        for k, v in package.items():
            self.assertEqual(str(getattr(resp2.context[0].get('paquete'),
                                         k)), v)
        
        resp_details = resp2.context[0].get('detalles')[0].get('Architectures')
        
        for k, v in details_i386.items():
            self.assertEqual(str(getattr(resp_details.get('i386').get('data'),
                                         k)), v)
        
        for k, v in details_amd64.items():
            self.assertEqual(str(getattr(resp_details.get('amd64').get('data'),
                                         k)), v)
