...
if [ "${TYPE}" == "final-release" ]; then
	...
	python -B tools/googlecode-upload.py -s "Paquete Debian [${NEWDEBVERSION}]" -p "canaima-semilla" -l "Featured,Type-Package,Type-Installer,OpSys-Linux,Stable" ../canaima-semilla_${NEWDEBVERSION}_all.deb
	python -B tools/googlecode-upload.py -s "Fuente Debian (dsc) [${NEWDEBVERSION}]" -p "canaima-semilla" -l "Type-Source,OpSys-Linux,Stable" ../canaima-semilla_${NEWDEBVERSION}.dsc
	python -B tools/googlecode-upload.py -s "Fuente Debian (debian.tar.gz) [${NEWDEBVERSION}]" -p "canaima-semilla" -l "Type-Archive,Type-Source,OpSys-Linux,Stable" ../canaima-semilla_${NEWDEBVERSION}.debian.tar.gz
	python -B tools/googlecode-upload.py -s "Fuente Original (orig.tar.gz) [${NEWRELVERSION}]" -p "canaima-semilla" -l "Featured,Type-Archive,Type-Source,OpSys-Linux,Stable" ../canaima-semilla_${NEWRELVERSION}.orig.tar.gz
fi
